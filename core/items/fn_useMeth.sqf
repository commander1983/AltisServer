...
// Overdose
if ( life_drugged_meth > 2) then
{
	_dmg = (damage player) + 0.5;
	life_drugged_meth = life_drugged_meth - 1;
...
resetCamShake;
	//Effekt Ende
	for [{_x=0},{_x < life_drugged_meth_duration * 4 && Alive player},{_x=_x+1}] do
	{
		if(_x % 4 == 0) then
		{
	...
	_count = 0;
	while{life_drugged_meth < 1 && Alive player} do 
	{
		if(life_drugged_meth < 0 OR _count > 100) exitWith { life_drugged_meth = -1; hint "Du bist nun nicht mehr suechtig.";};
	...
	if(life_drugged_meth < 0 || !Alive player) then { player setVariable["drug_meth",false,true]; life_drugged_meth = -1; };
