...
_resourceZones = ["apple_1","apple_2","apple_3","apple_4","peaches_1","peaches_2","peaches_3","peaches_4","gold_1","lead_1","iron_1","salt_1","sand_1","diamond_1","oil_1","oil_2","rock_1","silber_1","cerises_1","cerises_2","cerises_3","cerises_4","banane_1","banane_2","banane_3","banane_4","birne_1","birne_2","birne_3","birne_4","kristall_1"];
_zone = "";
...
if(_zone == "") exitWith {
	hint localize "STR_NOTF_DeviceNotNearResource";
...
_item = switch(true) do {
	...
	case (_zone in ["cerises_1","cerises_2","cerises_3","cerises_4"]): {"cerises"};
	case (_zone in ["banane_1","banane_2","banane_3","banane_4"]): {"banane"};
	case (_zone in ["birne_1","birne_2","birne_3","birne_4"]): {"birne"};
	case (_zone in ["gold_1"]): {"goldu"};
	...
	case (_zone in ["kristall_1"]): {"kristall"};
	default {""};
};
...
while {true} do {
	...
	if(isEngineOn _vehicle) exitWith {titleText[localize "STR_NOTF_DeviceMiningStopped","PLAIN"];};
	titleText[localize "STR_NOTF_DeviceMining","PLAIN"];
	_time = time + 27;
	
	//Wait for 27 seconds with a 'delta-time' wait.
	waitUntil {
		if(isEngineOn _vehicle) exitWith {titleText[localize "STR_NOTF_DeviceMiningStopped","PLAIN"]; true};
	...
	if(fuel _vehicle == 0) exitWith {titleText[localize "STR_NOTF_DeviceOutOfFuel","PLAIN"];};
