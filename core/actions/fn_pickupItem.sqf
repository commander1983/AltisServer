...
if((time - life_action_delay) < 2) exitWith {hint "Du kannst die Aktions-Taste nicht so schnell benutzen!"};
...
if(playerSide == west && _illegal != -1) exitWith
{
	...
	ja_pare = ja_pare + ((life_illegal_items select _illegal) select 1);
	deleteVehicle _obj;
