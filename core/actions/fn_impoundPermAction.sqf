...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if(count _vehicleData == 0) exitWith {hint "Das Fahrzeug wurde vermutlich durch Cheats gespawnt. Es wird gelöscht."; sleep 3; deleteVehicle _vehicle}; //Bad vehicle.
    [[0,format[localize "STR_NOTF_BeingImpoundedPerm",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
    life_action_inUse = true;
    
    _upp = localize "STR_NOTF_ImpoundingPerm";
    ...
    if(player distance _vehicle > 10) exitWith {hint localize "STR_NOTF_ImpoundingCancelledPerm"; life_action_inUse = false;};
    ...
    if((count crew _vehicle) == 0) then
    {
        ...
        hint format[localize "STR_NOTF_ImpoundedPerm",_type,_price];
        [[0,format[localize "STR_NOTF_HasImpoundedPerm",player getVariable["realname",name player],(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
