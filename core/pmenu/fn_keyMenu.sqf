...
{ if(player distance _x < 4) then {_near_units pushBack _x};} foreach playableUnits;
...
for "_i" from 0 to (count life_vehicles)-1 do
{
	...
	if(!isNull _veh && alive _veh) then
	{
		...
		_vehicles lbAdd format["%1 %3 - [Reichweite: %2m]",_name,round(player distance _veh),_text];
...
{
	_vehicles lbAdd "Kein Fahrzeug gefunden!";
