...
_funds = grpPlayer getVariable "gang_bank";	//Gang Bank Abfrage (damit auch von Gangkonto aus Slots bezahlen werden können)
_action = [
...
if(_action) then {
if((grpPlayer getVariable "gang_bank") < _upgradePrice) exitWith {	//vom Gangkonto
...
[_funds] call life_fnc_numberText,	//vom Gangkonto
[_upgradePrice - _funds] call life_fnc_numberText	//vom Gangkonto
];
};
__SUB__(_funds,_upgradePrice);	//vom Gangkonto
