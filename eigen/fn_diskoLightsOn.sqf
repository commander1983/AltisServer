/* 
	fn_diskolightsOn.sqf
*/
private ["_obj","_lightRd","_lightB","_lightL","_lightR","_leftRed","_lumens","_attach"];

_obj = _this select 0;
if (isNil "_obj" || isNull _obj || !(_obj getVariable "diskolights")) exitWith {};

 _attach = [[5, 0, 1.5], [-5, 0, 1.5],[0,5,1.5],[0,-5,1.5],[0,0,2.5]];
...
while {(alive _obj)} do
 {
	if (!(_obj getVariable "diskolights")) exitWith {};	
	_lightUp setLightBrightness 0;
	sleep 0.03;
